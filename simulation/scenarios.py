...
from core.algorithm import Agent, DistributedMPC,SearchGrid,MovingTarget, StaticTarget
...
def create_basic_scenario():
    ...
    # 添加静态目标
    grid.add_target_object(StaticTarget(position=(1000, 1000)))  # 西北角
    grid.add_target_object(StaticTarget(position=(4000, 4000)))  # 东南角
...
def create_boundary_priority_scenario():
    ...
    # 在边界添加多个目标
    grid.add_target_object(StaticTarget(position=(500, 500)))         # 西北
    grid.add_target_object(StaticTarget(position=(500, 4500)))       # 东北
    grid.add_target_object(StaticTarget(position=(4500, 500)))        # 西南
    grid.add_target_object(StaticTarget(position=(4500, 4500)))      # 东南
...
def create_moving_targets_scenario():
    """创建移动目标验证场景（2 UAV + 4 USV）"""
    grid = SearchGrid(width=5, height=5, resolution=0.1)
    center_y = 4630
    usv_spacing = 1000
    usv1_y = center_y - usv_spacing*1.5
    usv2_y = center_y - usv_spacing*0.5
    usv3_y = center_y + usv_spacing*0.5
    usv4_y = center_y + usv_spacing*1.5
    uav_spacing = 2000
    uav1_y = center_y - uav_spacing*0.5
    uav2_y = center_y + uav_spacing*0.5
    agents = [
        Agent("UAV1", position=(0, uav1_y), heading=90, agent_type="drone", safe_distance=50),
        Agent("UAV2", position=(0, uav2_y), heading=90, agent_type="drone", safe_distance=50),
        Agent("USV1", position=(0, usv1_y), heading=90, agent_type="usv", safe_distance=100),
    ...
    # 添加移动目标（初始化两个，演示用）
    target1 = MovingTarget(start_position=(1000, 1000))
    target2 = MovingTarget(start_position=(4000, 1000))
    grid.add_target_object(target1)
    grid.add_target_object(target2)
